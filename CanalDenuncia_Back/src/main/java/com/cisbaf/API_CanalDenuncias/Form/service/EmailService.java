...
public class EmailService {
    ...
    public void enviarEmail(String formularioHtml, boolean isUser, String emailUser) {
        try {
    ...
    public void enviarEmailNovaDenuncia(Denuncia denuncia) {
        enviarEmail(
            montarEmailDeNovaDenunciaAdmin(denuncia.getProtocolo()), 
            false,
            null
        );

        String emailUser = null;
        if (denuncia.getTipoDenunciante() == TipoDenunciante.VITIMA && denuncia.getVitima() != null) {
            emailUser = denuncia.getVitima().getEmail();
        } else if (denuncia.getTipoDenunciante() == TipoDenunciante.TERCEIRO && denuncia.getTerceiro() != null) {
            emailUser = denuncia.getTerceiro().getEmail();
        }

        if (emailUser != null && !emailUser.trim().isEmpty()) {
            enviarEmail(
                montarEmailDeNovaDenunciaUser(denuncia.getProtocolo()),
                true,
                emailUser
            );
        }
    ...
    private String montarEmailDeNovaDenunciaAdmin(String protocolo) {
    ...
    private String montarEmailDeNovaDenunciaUser(String protocolo) {
        LocalDate dataRegistro = LocalDate.now();
        String dataFormatada = dataRegistro != null ? dataRegistro.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) : "Não informada";
        ...
        return "<!DOCTYPE html>\n" +
               ...
               "        <div class=\"content\">\n" +
               "            <p>Olá,</p>\n" +
               "            <p>Sua denúncia foi recebida com sucesso em nosso sistema.</p>\n" +
               "            <div class=\"info-card\">\n" +
               "                <div class=\"info-item\">\n" +
               "                    <span class=\"info-label\">Número do Protocolo:</span>\n" +
               "                    <span class=\"info-value\">" + protocolo + "</span>\n" +
               ...
               "                <div class=\"info-item\">\n" +
               "                    <span class=\"info-label\">URL para Acompanhamento:</span>\n" +
               "                    <span class=\"info-value\">" + "<a href=\"" + frontUrl + "/protocolo\">Acompanhe sua denúncia</a>" + "</span>\n" +
               "                </div>\n" +
               "            </div>\n" +
               "        </div>\n" +
               "        <div class=\"footer\">\n" +
